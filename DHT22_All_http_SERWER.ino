...
String boardIPnumber[2][11] = {{"0", "53/temperature", "51/temperature", "50/temperature", "50/temperature1", "57/temperature", "56/temperature", "55/temperature", "58/temperature"},
  {"0", "53/humidity", "51/humidity", "50/humidity", "50/humidity1", "57/humidity", "56/humidity", "55/humidity", "58/humidity"}
};
String roomsname[11] = {"zielony","hol_pietro","oliwii","sypialnia","prysznic","salon","hol_parter","poddasze","pralnia","kotlowania","garaz"};

//Your Domain name with URL path or IP address with path
//roomsnames[0] = "zielony";
//roomsnames[1] = "hol_pietro";
//boardsStruct[2].id = "oliwii";
//boardsStruct[3].id = "sypialnia";
//boardsStruct[4].id = "prysznic";
//boardsStruct[5].id = "salon";
//boardsStruct[6].id = "hol_parter";
//boardsStruct[7].id = "poddasze";
//boardsStruct[8].id = "pralnia";
//boardsStruct[9].id = "kotlowania";
//boardsStruct[10].id = "garaz";
...
void handleAllReadings() {
  ...
  for (int x = 0; x < arrSize; x++) {
    AllboardsJSON[roomsname[x]]["temperature"] =  boardsStruct[x].temp;
    AllboardsJSON[roomsname[x]]["humidity"] =  boardsStruct[x].hum;
  }
  jsonString = JSON.stringify(AllboardsJSON);
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (isnan(newH)) {
      ...
      h = newH;
      Serial.println(h);
      boardsStruct[0].hum = h;
    }
    
    // Http Get method to collect
    WiFiClient client;
    HTTPClient http;
    for (int y = 0; y < 2; y++) {
      for (int x = 1; x < 11; x++) {
        if (boardIPnumber[y][x] != null ) {
          String serverPath = "http://192.168.1." + boardIPnumber[y][x];
          ...
          if (httpResponseCode > 0) {
            ...
            if (y == 0){
              boardsStruct[x].temp = payload;
            }
            else {
              boardsStruct[x].hum = payload;
            }
          }
          else {
